...
export default function ManageGenerationModal({
  ...
  const handleProjectClick = (file: any) => {
    if (!file.project_id) { // Se não tem project_id, é um projeto
      setSelectedProjectId(file.id);
      setSelectedFolderId(null);
      if (file.pastes) {
        setData(file.pastes as any);
        setIsInSubFolder(true);
      }
    } else { // Caso contrário, é uma pasta
      ...
      >
        <ModalContent>
          ...
          <ScrollShadow className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 gap-4 px-6 max-h-80 overflow-y-scroll scrollbar">
            ...
            />
            {data?.map((item: any) => {
              const { id, name, total_pastes = 0, total_generations = 0, pastes } = item;
              ...
              return (
                <FolderCard
                  ...
                  isSelected={selectedFolderId === id || selectedProjectId === id}
                />
              );
            })}
